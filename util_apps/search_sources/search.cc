...
int main(int argc, char **argv)
{
    ...
    if (argc < 5) {
	cout << "usage: " << argv[0] <<
		" <path to database> <start result #> <end result #> <search terms>" << endl;
	exit(1);
    }
	...
	// Get the results (starting at the start and display end_num results from start
	MSet matches = enquire.get_mset(start_num,end_num);

	// Display the results
//	cout << matches.get_matches_estimated() << " results found" << endl;

	for (MSetIterator i = matches.begin(); i != matches.end(); ++i) {
	    cout << i.get_percent() << "%" << endl
