        ...
        if form.has_key('logout') or isdisabled:
		    ...
		    cookie_dir = config.web_dir
	   	    if not cookie_dir: cookie_dir = '/'
                    self.request.setHttpHeader('Set-Cookie: %s=%s; expires=Tuesday, 01-Jan-1999 12:00:00 GMT;domain=%s;Path=%s' % (wikiutil.quoteFilename(config.sitename)+'ID', cookie[wikiutil.quoteFilename(config.sitename)+'ID'].value,config.domain, cookie_dir))
...
""" % (config.relative_dir, add_on, config.relative_dir, add_on)
            ...
            for key, label in user.User._checkbox_fields:
                ...
                try:
                    ...
                    pass
		if newuser:
		    setattr(theuser, 'show_nonexist_qm', 0)
                else:
    ...
    def asHTML(self):
        ...
        if self.request.user.valid:
            ...
            for key, label in checkbox_fields:
                if key != 'show_nonexist_qm' and key != "wikiname_add_spaces" and key !="show_emoticons" and key !="show_page_trail" and key !="show_toolbar" and key !="show_topbottom" and key != "show_fancy_links" and key != "show_fancy_diff" and key != "remember_last_visit":
                    bool_options.extend([
                        html.INPUT(type="checkbox", name=key, value=1,
                            checked=getattr(self.request.user, key, 0)),
                        ' ', label(_), html.BR(),
                     ])
	#	elif key is 'show_nonexist_qm':
         #           bool_options.extend([
          #              html.INPUT(type="checkbox", name=key, value=1,
           #                 checked=getattr(self.request.user, key, 1)),
            #            ' ', label(_), html.BR(),
             #        ])
