...
""" % dict
    ...
    def showversion(self, d, **keywords):
        ...
        if config.show_version and not keywords.get('print_mode', 0):
            html = ('<p>'
                    'LocalWiki %s, Copyright \xa9 2000-2004 by Jürgen Hermann'
                    '</p>' % (version.revision,))
        return html
    ...
    def recentchanges_entry(self, d):
        ...
        if d['show_comments'] == 0:
            ...
            if not com:
              ...
              com = '(' + com + ')'
            html.append(' by </span><span class="rceditor">%s</span> <span class="rccomment">%s' % (d['editors'][0],com ))
        html.append('</span></td>\n')
        html.append('</tr>')
        ...
        if d['editors'] and d['show_comments'] == 1:
            for editor in d['editors']:
                  com = d['comments'][num]
                  if not com:
                    com = '(No comment)'
                  html.append('<tr><td>&nbsp;</td><td class="rccomment">%s&nbsp;&nbsp;<span class="rceditor">%s</span></td></tr>' % (com, editor))
                  num = num + 1
