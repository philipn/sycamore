...
#                raise OSError
	...
	if self.hilite_re:
		request.write('<table width="100%%"><tr><td align="right">[<strong class="highlight"><a href="./%s">highlighting off</a></strong>]</td></tr></table>' % self.page_name)
    ...
    def getPageLinks(self, request, docache=True):
        ...
        @param request: the request object
	@param docache:  set to False to make this fast for macro functions, otherwise it might create the cache ona whole number of pages, redirecting the request object and causing trouble
        @rtype: list
        ...
        key   = wikiutil.quoteFilename(self.page_name)
        cache = caching.CacheEntry(arena, key)
        if cache.needsUpdate(self._text_filename()) and docache:
	...
	ourtext = cache.content()
	if not ourtext:
		return ''
	else:
        	return filter(None, ourtext.split('\n'))
