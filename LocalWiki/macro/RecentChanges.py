...
def format_page_edits(macro, lines, showcomments, bookmark):
    ...
    if config.show_hosts:
        if len(lines) > 1:
            ...
            d['editors'] = []
            for name in editorlist:
                d['editors'].append("%s" % name)
        else:
            d['editors'] = []
            d['editors'].append(line.getEditor(request))

    comments = []
    for idx in range(len(lines)):
        comment = format_comment(request, lines[idx])
        comments.append(wikiutil.escape(comment))
...
def execute(macro, args, **kw):
    ...
    for line in log.reverse():
        ...
        if (((this_day != day or (not hilite and not max_days)))
            ...
            for page in pages:
                request.write(format_page_edits(macro, page, showComments, bookmark))
            day_count += 1
            pages = {}
