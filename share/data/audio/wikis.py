...
# only worry about these if you're importing more than one wiki
wiki_id = 1
wiki_name = 'yourwikinamehere'
...
def rebuild_all_caches(req):
    for wiki_name in wikiutil.getWikiList(req):
...
def convert_user_links(request):
    ...
    """
    def _fix_text(text, request):
        fixed = []
        offset = 0
        for match in re.finditer('\["(?P<pagename>[^"]+)".*?\]', text):
            pagename = match.group('pagename')
            isuser = user.User(request, name=pagename.lower())
            if isuser.valid:
                text = text[:match.start('pagename')+offset] + config.user_page_prefix + pagename + text[match.end('pagename')+offset:]
                offset += len(config.user_page_prefix)
        return text
    ...
    for wikiname in wikiutil.getWikiList(request):
        ...
        for page in wikiutil.getPageList(request, objects=True):
            ...
            text = _fix_text(text, request)
            d = {'newtext': text, 'pagename': page.page_name, 'wiki_id': request.config.wiki_id}
            request.cursor.execute("UPDATE curPages set text=%(newtext)s where name=%(pagename)s and wiki_id=%(wiki_id)s", d, isWrite=True)
            request.cursor.execute("UPDATE allPages set text=%(newtext)s where name=%(pagename)s and wiki_id=%(wiki_id)s", d, isWrite=True)

def fix_user_tz(request):
    request.cursor.execute("UPDATE users set tz=%(global_tz)s", {'global_tz': request.config.tz}, isWrite=True)
...
def _user_page_move(request, d):
    ...
    request.cursor.execute("SELECT name, attached_to_pagename, attached_to_pagename_propercased from files where wiki_id=%(wiki_id)s", d)
    result = request.cursor.fetchall()
    if result:
        # we want to preserve the subtle differences in old page proper names, hense the iteration here
    ...
    request.cursor.execute("SELECT name, attached_to_pagename, attached_to_pagename_propercased, uploaded_time from oldFiles where wiki_id=%(wiki_id)s", d)
    result = request.cursor.fetchall()
    if result:
        # we want to preserve the subtle differences in old page proper names, hense the iteration here
    ...
    request.cursor.execute("SELECT pagename, pagename_propercased, x, y from mapPoints where wiki_id=%(wiki_id)s", d)
    result = request.cursor.fetchall()
    if result:
        # we want to preserve the subtle differences in old page proper names, hense the iteration here
    ...
    request.cursor.execute("SELECT pagename, pagename_propercased, x, y, deleted_time from oldMapPoints where wiki_id=%(wiki_id)s", d)
    result = request.cursor.fetchall()
    if result:
        # we want to preserve the subtle differences in old page proper names, hense the iteration here
...
def fix_mapPoints(request):
    ...
    request.cursor.execute("SELECT (pagename, x, y, created_time, created_by, created_by_ip, id, pagename_propercased, address) from events where wiki_id=%(wiki_id)s", d)
    results = request.cursor.fetchall()
    if results: 
      request.cursor.execute("DELETE from mapPoints where wiki_id=%(wiki_id)s", d, isWrite=True)
      for result in results: 
        result_dict = {'pagename': result[0], 'x': result[1], 'y': result[2], 'created_time': result[3], 'created_by': result[4], 'created_by_ip': result[5], 'id': result[6] + maps_start, 'pagename_propercased': result[7], 'address': result[8]}
        request.cursor.execute("INSERT into mapPoints (pagename, x, y, created_time, created_by, created_by_ip, id, pagename_propercased, address, wiki_id) values (%(pagename)s, %(x)s, %(y)s, %(created_time)s, %(created_by)s, %(created_by_ip)s, %(id)s, %(pagename_propercased)s, %(address)s)", result_dict, isWrite=True)
...
req.cursor.execute("UPDATE users set tz=NULL;", isWrite=True)
...
req.cursor.execute("ALTER TABLE userFavorites ADD PRIMARY KEY (username, page, wiki_id);", isWrite=True)
...
req.cursor.execute("ALTER TABLE mapPointCategories ADD PRIMARY KEY (pagename, x, y, wiki_id);", isWrite=True)
...
req.cursor.execute("ALTER TABLE oldMapPointCategories ADD PRIMARY KEY (pagename, x, y, deleted_time, wiki_id);", isWrite=True)
...
if config.db_type == 'mysql':
  ...
  (
    ...
    primary key (wiki_name, hashed_code, written_time)
  )""", isWrite=True)
