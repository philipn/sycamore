...
def make_random_string(length, alphanum_only=True):
    ...
    chars = string.letters + string.digits # not everything, but good nuff 4now
...
def _create_ip():
    return '%s.%s.%s.%s' % (random.randint(0, 255), random.randint(0,255),
...
class BasicUserGroups(RequestBasics):
    ...
    def testGroupSave(self):
        ...
        # cleanup...we can fix this by putting every test in a class derived
        # from RequestBasics
    ...
    def testSetDefaultRights(self):
        ...
        def _create_rights(groups):
            ...
            for groupname in groups:
                if groupname in ["Admin", "All", "Known", "Banned"]:
                    continue
                rights[groupname] = (random.choice([True, False]),
                                     random.choice([True, False]),
                                     random.choice([True, False]),
                                     random.choice([True, False]))
            rights['All'] = (random.choice([True, False]),
                             random.choice([True, False]),
                             random.choice([True, False]),
                             False)
            rights['Known'] = (random.choice([True, False]),
                               random.choice([True, False]),
                               random.choice([True, False]),
                               False)
            rights['Banned'] = (random.choice([True, False]),
                                random.choice([True, False]),
                                random.choice([True, False]),
                                False)
        ...
        def _check_page_rights(rights):
            page = Page('this page has never existed.%s' % time.time(),
                        self.request)
            for groupname in rights.keys():
                 ...
                 random_user = user.User(self.request,
                                         name=random_username_in_group)
                 ...
                 for iter_groupname in _remove_preset_groups(rights):
                    iter_group = wikiacl.Group(iter_groupname, self.request,
                                               fresh=True)
                    if random_user.name in iter_group:
                        user_is_in_a_group = True
                        may_read_status = (may_read_status or
                            rights[iter_groupname][ACL_RIGHTS_TABLE['read']])
                        may_edit_status = (may_edit_status or
                            rights[iter_groupname][ACL_RIGHTS_TABLE['edit']])
                        may_delete_status = (may_delete_status or
                            rights[iter_groupname][ACL_RIGHTS_TABLE['delete']])
                        may_admin_status = (may_admin_status or
                            rights[iter_groupname][ACL_RIGHTS_TABLE['admin']])

                 if not user_is_in_a_group:
                    # Set up the default rights for the user
                    may_read_status = (
                        rights['All'][ACL_RIGHTS_TABLE['read']] or
                        rights['Known'][ACL_RIGHTS_TABLE['read']])
                    may_edit_status = (
                        rights['All'][ACL_RIGHTS_TABLE['edit']] or
                        rights['Known'][ACL_RIGHTS_TABLE['edit']])
                    may_delete_status = (
                        rights['All'][ACL_RIGHTS_TABLE['delete']] or
                        rights['Known'][ACL_RIGHTS_TABLE['delete']])
                    may_admin_status = (
                        rights['All'][ACL_RIGHTS_TABLE['admin']] or
                        rights['Known'][ACL_RIGHTS_TABLE['admin']])

                 if random_user.name in wikiacl.Group('Banned', self.request,
                                                      fresh=True):
                        may_read_status = \
                            rights['Banned'][ACL_RIGHTS_TABLE['read']]
                        may_edit_status = \
                            rights['Banned'][ACL_RIGHTS_TABLE['edit']]
                        may_delete_status = \
                            rights['Banned'][ACL_RIGHTS_TABLE['delete']]
                        may_admin_status = \
                            rights['Banned'][ACL_RIGHTS_TABLE['admin']]
                                  
                 if random_user.name in wikiacl.Group('Admin', self.request,
                                                      fresh=True):
                       # admins are gods
                 ...
                 self.assertEqual(may_read_status,
                    random_user.may.read(page, fresh=True))
                 self.assertEqual(may_admin_status,
                    random_user.may.admin(page, fresh=True))
                 self.assertEqual(may_edit_status,
                    random_user.may.edit(page, fresh=True))
                 self.assertEqual(may_delete_status,
                    random_user.may.delete(page, fresh=True))
        ...
        for i in range(0, 10):
            ...
            wiki_config = config.Config(self.request.config.wiki_name,
                                        self.request, fresh=True)
            self.assertEqual(wiki_config.acl_rights_default, rights)
            # test to make sure that a wiki page w/o custom rights has
            # these default rights.
