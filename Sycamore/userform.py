...
class UserSettingsHandler(object):
        ...
        if config.memcache:
            ...
            if obj:
                ...
                if (stored_secret and
                    (stored_secret == user.hash(cookie_secret))):
                    self.request.mc.delete(key, wiki_global=True)
        self.request.cursor.execute(
            """DELETE from userSessions
               where user_id=%(cookie_userid)s and
                     session_id=%(cookie_sessionid)s and
                     secret=%(cookie_secret)s""",
            {'cookie_userid':cookie_userid,
             'cookie_sessionid':cookie_sessionid,
             'cookie_secret':user.hash(cookie_secret)}, isWrite=True)
        ...
        self.request.cursor.execute(
            """DELETE from userSessions
               where user_id=%(user_id)s and
                     session_id != %(current_session)s""",
            {'user_id':self.request.user.id,
             'current_session':current_session}, isWrite=True)
        # clear from memcache
        for session in results:
            session_id = session[0]
            self.request.mc.delete("userSessions:%s,%s" % (
                self.request.user.id, session_id))

    def isValidCode(self, given_uid, given_code):
        state = False
        timenow = time.time()
        d = {'uid':given_uid, 'code':given_code,
             'wiki_id':self.request.config.wiki_id,
             'timevalid': (timenow-60*30)}
        self.request.cursor.execute(
            """SELECT written_time from lostPasswords
               where code=%(code)s and uid=%(uid)s and
                     written_time > %(timevalid)s""", d)
        result = self.request.cursor.fetchone()
        if result and result[0]:
            state = True
            self.request.cursor.execute(
                """DELETE from lostPasswords where uid=%(uid)s""",
                d, isWrite=True)
        return state
    ...
    def createCode(self, userid):
        ...
        self.request.cursor.execute(
            """INSERT into lostPasswords (uid, code, written_time)
               values (%(uid)s, %(code)s, %(written_time)s)""",
            d, isWrite=True)
...
def send_back_home(request, msg=''):
    if (request.form.has_key('backto_wiki') and
        request.form.has_key('backto_page')):
        ...
        if q_query_string:
            url = ('%s?action=userform&backto_page=%s&qs=%s&secret=%s'
                   '&expire_time=%s&uid=%s&session=%s' % (
                    urllib.unquote(q_backto_page), q_backto_page,
                    q_query_string, urllib.quote(secret), stored_expire_time,
                    request.user.id, urllib.quote(session)))
        else:
             url = ('%s?action=userform&backto_page=%s&secret=%s'
                    '&expire_time=%s&uid=%s&session=%s' % (
                    urllib.unquote(q_backto_page), q_backto_page,
                    urllib.quote(secret), stored_expire_time, request.user.id,
                    urllib.quote(session)))
...
user_checkbox_fields = {
    'remember_me':('Remember login information (so you don\'t'
                   'have to keep logging in'),
    'disabled':('Disable this account forever &mdash; WARNING!! &mdash; '
                'permanent')}

first_time_msg = ("<h2>First time</h2>"
                  "<p>"
                  "Your email is needed for you to be able to recover lost "
                  "login data.</p>"
                  "<p>If you click on Create Profile, a user profile will be "
                  "created for you and you will be logged in immediately.</p>")

forgot_password_msg = ("<h2>Forgot password?</h2>"
                       "<p>If you forgot your password, attempt to log in "
                       "via the login box in the upper right hand corner "
                       "of the screen and you will be given further "
                       "instruction.</p>")
        ...
        if not option_text:
            self._inner.append(html.DIV(
                html_class="settings_form_item").extend([
                    html.DIV(
                        html_class="settings_form_label",
                        **kw).extend([label]),
...
# 10 years, after this time the polar icecaps will have melted anyway
