...
'''For at oprette dine egne skabeloner skal du tilføje en side der matcher det regulære udtryk (regex) "%(page_template_regex)s".''',
'''The following pages with similar names already exist...''':
'''Følgende sider med lignende navne eksisterer allerede...''',
...
''':
'''<dt>Fremhævelser:</dt>
<dd>\'\'<em>kursiv</em>\'\'; \'\'\'<strong>fed</strong>\'\'\'; \'\'\'\'\'<strong><em>kursiv fed</em></strong>\'\'\'\'\';
    \'\'<em>blandet \'\'\'<strong>fed</strong>\'\'\' og kursiv</em>\'\'; ---- vandret streg.</dd>
<dt>Overskrifter:</dt>
<dd>= Titel 1 =; == Titel 2 ==; === Titel 3 ===;
    ==== Titel 4 ====; ===== Titel 5 =====.</dd>
<dt>Lister:</dt>
<dd>mellemrum og en asterisk (*); 1., a., A., i., I. nummererede emner;
    1.#n begynd nummerering ved n; mellemrum alene giver indryk.</dd>
<dt>Links:</dt>
<dd>SammenSatteOrd; ["lodrette klammer og citationstegn"];
    url; [url]; [url beskrivende navn].</dd>
<dt>Tabeller:</dt>
<dd>|| celletekst |||| celletekst over to kolonner ||;
    efterfølgende mellemrum er ikke tiladt efter tabeller eller titler.</dt>
''',
'''Edit was cancelled.''':
...
rettelser derfra. Vælg derefter RedigérTekst igen, og tilføj dine rettelser til den
nuværende tekst.</p>
<p><em>Overskriv ikke blot den nuværende tekst med din version. Det vil slette
den anden persons rettelser, og det er meget ubehøvlet!!</em></p>
''',
'''A backup of your changes is <a href="%(backup_url)s">here</a>.''':
'''En sikkerhedskopi af dine rettelser er <a href="%(backup_url)s">her</a>.''',
...
'''Andre brugere vil være <em>udelukket</em> fra at redigere denne side indtil %(bumptime)s.''',
'''Other users will be <em>warned</em> until %(bumptime)s that you are editing this page.''':
'''Andre brugere vil blive <em>advaret</em> indtil %(bumptime)s om at du redigerer denne side.''',
...
'''Denne side er i øjeblikket <em>låst</em> for redigering af %(owner)s indtil %(timestamp)s, dvs. i %(mins_valid)d minut(ter).''',
...
<strong> class="highlight">Du bør <em>undgå at redigere</em> denne side ihvertfald %(mins_valid)d minut(ter) endnu,
for at undgå redigeringskonflikter.</strong><br>
...
'''Husk adgangsinformationer permanent''',
...
'''Formulér venligst søgningen mere specifikt end \'%(needle)s\'!''',
'''Full text search for "%s"''':
'''Fritekst-søgning efter "%s"''',
'''match''':
'''resultat''',
'''matches''':
'''resultater''',
...
'''%(hits)d resultater ved søgning i %(pages)d sider.''',
...
'''Forskelle mellem versionerne dateret %s og %s''',
'''(spanning %d versions)''':
'''(over %d versioner)''',
'''No differences found!''':
'''Ingen forskelle fundet!''',
...
'''Tilbagefør til version dateret %(datestamp)s.''',
'''N/A''':
'''Ikke tilgængelig''',
'''Info for "%s"''':
'''Info om "%s"''',
...
'''Der opstod en fejl under tilbageførsel af siden.''',
...
'''Du abonnerer allerede på denne side.''',
...
'''<p>Brug <strong><tt>attachment:filename</tt></strong> til at referere til vedhæftede
filer på en side, som vist nedenfor i fil-oversigten.
Anvend <strong>IKKE</strong> URL\'en under <tt>[get]</tt>, da den link kun er midlertidig
og derfor kan ødelægge integriteten af informationerne.</p>''',
'''del''':
'''slet''',
'''get''':
'''hent''',
...
'''Du kan ikke bruge "LikePages" på et udvidet sidenavn!''',
...
'''Siden "%s" blev korrekt omdøbt til "%s"!''',
'''Rename''':
'''Omdøb''',
'''New name''':
'''Nyt navn''',
...
'''Opdatér datostemplet for mit bogmærke''',
'''set bookmark''':
'''sæt bogmærke''',
'''[Bookmark reached]''':
'''[Bogmærke nået]''',
...
'''Vis alle ændringer de seneste %s dage.''',
'''Line''':
'''Linje''',
