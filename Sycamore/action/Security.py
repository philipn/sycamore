"""
    ...
    This action allows you set specific security settings on an individual page.
...
# Imports
from Sycamore import config, user, wikiacl
...
def execute(pagename, request):
    ...
    if not request.user.may.admin(page):
        msg = _("""You don't have admin permissions on this page, so you cannot change security settings.""")
    ...
    if request.form.has_key('button'):
        ...
        for key in request.form:
          if key.endswith('_groupname'): groups.append(unquoteWikiname(key[:-10]))
          else:
        ...
        # set groups we weren't sent any checkboxes for to all false (nothing checked)
        groups_no_checks = filter(lambda(groupname): (groupname not in groups_dict), groups)
...
""" % d)
    ...
    for groupname in grouplist:
      ...
      if group.may(page, 'read'):
        formhtml.append('<input type="checkbox" checked name="%s_may_read" value="1">' % quoteWikiname(groupname))
      else:
        formhtml.append('<input type="checkbox" name="%s_may_read" value="1">' % quoteWikiname(groupname))
      formhtml.append(' read ')

      if group.may(page, 'edit'):
        formhtml.append('<input type="checkbox" checked name="%s_may_edit" value="1">' % quoteWikiname(groupname))
      else:
        formhtml.append('<input type="checkbox" name="%s_may_edit" value="1">' % quoteWikiname(groupname))
      formhtml.append(' edit ') 

      if group.may(page, 'delete'):
        formhtml.append('<input type="checkbox" checked name="%s_may_delete" value="1">' % quoteWikiname(groupname))
      else:
    ...
    # wikiacl.Group(groupname, request)
    formhtml.append("""<p><input type="submit" name="button" value="%(button)s"></p>
</form>""" % d)
