...
class CacheEntry:
    ...
    def _consider_talk_link(self, links):
        ...
        if page.isTalkPage():
            ...
            # add dependency so that editing/creating the talk page has an affect on the article page's links
    ...
    def update(self, content, links):
        ...
        self.request.cursor.execute("UPDATE curPages set cachedText=%(cached_content)s, cachedTime=%(cached_time)s where name=%(key)s and wiki_id=%(wiki_id)s", {'cached_content':wikidb.dbapi.Binary(content), 'cached_time':cached_time, 'key':self.key, 'wiki_id':self.request.config.wiki_id}, isWrite=True)
        self.request.cursor.execute("DELETE from links where source_pagename=%(key)s and wiki_id=%(wiki_id)s", {'key':self.key, 'wiki_id':self.request.config.wiki_id}, isWrite=True)
        for link in links:
        ...
        if config.memcache:
           if self.request.set_cache:
             self.request.mc.set("page_info:%s" % wikiutil.mc_quote(self.key), page_info)
           else:
    ...
    def clear(self, type=None):
        ...
        self.request.cursor.execute("UPDATE curPages set cachedText=NULL, cachedTime=NULL where name=%(key)s and wiki_id=%(wiki_id)s", {'key':self.key, 'wiki_id':self.request.config.wiki_id}, isWrite=True)
        ...
        if self.request.req_cache['page_info'].has_key((key, self.request.config.wiki_id)):
            del self.request.req_cache['page_info'][(key, self.request.config.wiki_id)]
        if self.request.req_cache['pagenames'].has_key((self.key, self.request.config.wiki_id)):
            del self.request.req_cache['pagenames'][(self.key, self.request.config.wiki_id)]
        if config.memcache and self.key == self.request.config.interwikimap.lower():
...
def dependency(depend_pagename, source_pagename, request):
  ...
  request.cursor.execute("SELECT page_that_depends from pageDependencies where page_that_depends=%(depend_pagename)s and source_page=%(source_pagename)s and wiki_id=%(wiki_id)s", {'depend_pagename':depend_pagename, 'source_pagename':source_pagename, 'wiki_id':request.config.wiki_id})
  result = request.cursor.fetchone()
  if not result:
    request.cursor.execute("INSERT into pageDependencies (page_that_depends, source_page, wiki_id) values (%(depend_pagename)s, %(source_pagename)s, %(wiki_id)s)", {'depend_pagename':depend_pagename, 'source_pagename':source_pagename, 'wiki_id':request.config.wiki_id}, isWrite=True)
  # if C <- B and B <- A then C <- A
  for i in range(0, MAX_DEPENDENCY_DEPTH):
    request.cursor.execute("SELECT source_page from pageDependencies where page_that_depends=%(source_pagename)s and wiki_id=%(wiki_id)s", {'depend_pagename':depend_pagename, 'source_pagename':source_pagename, 'wiki_id':request.config.wiki_id})
...
def clear_dependencies(pagename, request):
  ...
  request.cursor.execute("DELETE from pageDependencies where page_that_depends=%(page_name)s and wiki_id=%(wiki_id)s", {'page_name':pagename, 'wiki_id':request.config.wiki_id}, isWrite=True)
...
def depend_on_me(pagename, request, exists, action=None):
  ...
  request.cursor.execute("SELECT page_that_depends from pageDependencies where source_page=%(page_name)s and wiki_id=%(wiki_id)s", {'page_name':pagename, 'wiki_id':request.config.wiki_id})
  results = request.cursor.fetchall()
  page_deps = []
  for result in results:
  ...
  if do_links:
    request.cursor.execute("SELECT source_pagename from links where destination_pagename=%(page_name)s and wiki_id=%(wiki_id)s", {'page_name':pagename, 'wiki_id':request.config.wiki_id})
    for result in request.cursor.fetchall():
      page_deps.append(result[0])
  return page_deps

class pageInfoObj(object):
  def __init__(self, edit_info, cached_text, meta_text, has_acl, has_map=None):
...
def pageInfo(page, get_from_cache=True, cached_content=None, cached_time=None):
  ...
  if get_from_cache:
    ...
    if page.request.req_cache['page_info'].has_key((key, page.request.config.wiki_id)):
      return page.request.req_cache['page_info'][(key, page.request.config.wiki_id)]
  ...
  if page.exists():
    if not page.prev_date:
      page.cursor.execute("SELECT editTime, userEdited from curPages where name=%(page_name)s and wiki_id=%(wiki_id)s", {'page_name':page.page_name, 'wiki_id':page.request.config.wiki_id})
      ...
      page.cursor.execute("SELECT userEdited from allPages where name=%(page_name)s and editTime=%(date)s and wiki_id=%(wiki_id)s", {'page_name':page.page_name, 'date':page.prev_date, 'wiki_id':page.request.config.wiki_id})
      result = page.cursor.fetchone()
      editUserID = result[0]
    ...
    if not page.prev_date:
      if not cached_content or not cached_time:
        page.cursor.execute("SELECT cachedText, cachedTime from curPages where name=%(page)s and wiki_id=%(wiki_id)s", {'page':page.page_name, 'wiki_id':page.request.config.wiki_id})
        result = page.cursor.fetchone()
        if result:
  ...
  if not page.prev_date:
    if not config.has_old_wiki_map:
      ...
      page.cursor.execute("SELECT count(pagename) from mapPoints where pagename=%(page_name)s and wiki_id=%(wiki_id)s", {'page_name':page.page_name, 'wiki_id':page.request.config.wiki_id})
      result = page.cursor.fetchone()
      if result:
        if result[0]:
      ...
      if page.request.save_time: # we are in a 'saving' request
        ...
        if result:
          if result[0]:
            has_map = True
    if not page.exists():
       page.cursor.execute("SELECT latestEdit.editTime, allPages.userEdited from (SELECT max(editTime) as editTime from allPages where name=%(page_name)s and wiki_id=%(wiki_id)s) as latestEdit, allPages where allPages.name=%(page_name)s and allPages.editTime=latestEdit.editTime and allPages.wiki_id=%(wiki_id)s", {'page_name':page.page_name, 'wiki_id':page.request.config.wiki_id})
       result = page.cursor.fetchone()
       if result:
         editUserID = result[1]
         editTimeUnix = result[0]
  ...
  page.cursor.execute("SELECT groupname, may_read, may_edit, may_delete, may_admin from pageAcls where pagename=%(pagename)s and wiki_id=%(wiki_id)s", {'pagename':page.page_name, 'wiki_id':page.request.config.wiki_id})
  if page.cursor.fetchone(): has_acl = True
  else: has_acl = False
...
def getPageLinks(pagename, request, update=False):
  ...
  for link in links:
    ...
    if exists: request.req_cache['pagenames'][(key, request.config.wiki_name)] = proper_name
    else: request.req_cache['pagenames'][(key, request.config.wiki_name)] = False
...
def deleteAllPageInfo(pagename, request):
  ...
  if config.memcache:
      ...
      request.cursor.execute("SELECT editTime from allPages where name=%(pagename)s and wiki_id=%(wiki_id)s", d) 
      results = request.cursor.fetchall()
      for result in results:
        version = result[0] 
        # we do set() rather than delete() to avoid possible race conditions
        request.mc.set("page_text:%s,%s" % (wikiutil.mc_quote(pagename), repr(version)), False)
...
def deleteNewerPageInfo(pagename, version, request):
  ...
  """
  if config.memcache:
    d = {'pagename':pagename, 'version':version, 'wiki_id':request.config.wiki_id}    
    request.cursor.execute("SELECT editTime from allPages where name=%(pagename)s and wiki_id=%(wiki_id)s and editTime>%(version)s", d)
    results = request.cursor.fetchall()
    for result in results:
      newer_version = result[0]
      # we do set() rather than delete() to avoid possible race conditions
      request.mc.set("page_text:%s,%s" % (wikiutil.mc_quote(pagename), repr(newer_version)), False)
...
def deleteAllFileInfo(filename, pagename, request):
  ...
  """ 
  if config.memcache:
    d = {'filename':filename, 'pagename':pagename, 'wiki_id':request.config.wiki_id}
    # we do set() rather than delete() to avoid possible race conditions
    request.mc.set("oldfiles:%s,%s" % (wikiutil.mc_quote(filename), wikiutil.mc_quote(pagename)), False)
    request.cursor.execute("SELECT uploaded_time from oldFiles where name=%(filename)s and attached_to_pagename=%(pagename)s and wiki_id=%(wiki_id)s", d)
    results = request.cursor.fetchall()
    for result in results:
      version = result[0]
      # we do set() rather than delete() to avoid possible race conditions
      request.mc.set("oldfiles:%s,%s,%s" % (wikiutil.mc_quote(filename), wikiutil.mc_quote(pagename), repr(version)), False)
...
def deleteNewerFileInfo(filename, pagename, version, request):
  ...
  """ 
  if config.memcache:
    d = {'filename':filename, 'pagename':pagename, 'version': version, 'wiki_id':request.config.wiki_id}
    # we do set() rather than delete() to avoid possible race conditions
    request.cursor.execute("SELECT uploaded_time from oldFiles where name=%(filename)s and attached_to_pagename=%(pagename)s and wiki_id=%(wiki_id)s and uploaded_time>%(version)s", d)
    results = request.cursor.fetchall()
    for result in results:
      newer_version = result[0]
      # we do set() rather than delete() to avoid possible race conditions
      request.mc.set("oldfiles:%s,%s,%s" % (wikiutil.mc_quote(filename), wikiutil.mc_quote(pagename), repr(newer_version)), False)
