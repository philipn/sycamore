...
def execute(macro, args, formatter=None):
    ...
    icon_url = macro.request.getQualifiedURL(
        '/?action=mozilla_search&amp;file=%s.png' %
        macro.request.config.wiki_name)
    if icon_url:
        icon_link = '<img src="%s"/>&nbsp;' % icon_url
    else:
        icon_link = ''
    d = {'src_url':macro.request.getQualifiedURL(
            '/?action=mozilla_search&amp;file=%s.src' %
            macro.request.config.wiki_name), 
...
<a href="javascript:addEngine(\'%(wikiname)s\',\'Web\')">
%(icon_link)sInstall the %(sitename)s search plugin!</a>""" % d)
