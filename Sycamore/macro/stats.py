...
def execute(macro, args, formatter=None):
    ...
    if args:
       ...
       if sort_by == 'first_edit_date':
         cursor.execute("SELECT users.propercased_name, userWikiInfo.first_edit_date, userWikiInfo.created_count, userWikiInfo.edit_count, userWikiInfo.file_count, userWikiInfo.last_page_edited, userWikiInfo.last_edit_date, userWikiInfo.first_edit_date IS NULL AS join_isnull from userWikiInfo, users where users.name!='' and users.name=userWikiInfo.user_name and userWikiInfo.wiki_id=%%(wiki_id)s order by join_isnull ASC, %s desc" % sort_by, {'wiki_id':macro.request.config.wiki_id})
       elif sort_by == 'last_edit_date':
         cursor.execute("SELECT users.propercased_name, userWikiInfo.first_edit_date, userWikiInfo.created_count, userWikiInfo.edit_count, userWikiInfo.file_count, userWikiInfo.last_page_edited, userWikiInfo.last_edit_date, userWikiInfo.last_edit_date IS NULL AS edit_isnull from users, userWikiInfo where users.name!='' and users.name=userWikiInfo.user_name and userWikiInfo.wiki_id=%%(wiki_id)s order by edit_isnull ASC, %s desc" % sort_by, {'wiki_id':macro.request.config.wiki_id})
       else:
         cursor.execute("SELECT users.propercased_name, userWikiInfo.first_edit_date, userWikiInfo.created_count, userWikiInfo.edit_count, userWikiInfo.file_count, userWikiInfo.last_page_edited, userWikiInfo.last_edit_date from users, userWikiInfo where users.name!='' and users.name=userWikiInfo.user_name and userWikiInfo.wiki_id=%%(wiki_id)s order by %s desc" % sort_by, {'wiki_id':macro.request.config.wiki_id})
