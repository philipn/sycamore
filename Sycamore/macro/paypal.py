    ...
    if not args:
        return formatter.rawHTML(
            ...
            '<input type="hidden" name="business" value="' +
                config.paypal_address +'"> '
            '<input type="hidden" name="item_name" value="' +
                config.paypal_name + ' c/o ' +
                macro.request.config.sitename + '"> '
            '<input type="hidden" name="item_number" value="1"> '
            '<input type="hidden" name="no_note" value="1"> '
            '<input type="hidden" name="currency_code" value="USD"> '
            '<input type="hidden" name="tax" value="0"> '
            '<input type="image" src="https://www.paypal.com/en_US/i/btn/'
                                     'x-click-but21.gif" border="0" '
    ...
    if args:
        ...
        if re.match("^[_a-z0-9-]+(\.[_a-z0-9-]+)"
            ...
            return formatter.rawHTML(
                ...
                '<input type="hidden" name="business" value="' +
                    paypal_address +'"> '
                '<input type="hidden" name="item_name" value="'+ cobrand +'"> '
                '<input type="hidden" name="item_number" value="1"> '
                '<input type="hidden" name="no_note" value="1"> '
                '<input type="hidden" name="currency_code" value="USD"> '
                '<input type="hidden" name="tax" value="0"> '
            ...
            return formatter.rawHTML(
                '<strong>'
                'The Email address you specified seems malformed: ' + 
                    paypal_address + '</strong>')
