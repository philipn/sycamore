...
from Sycamore import config, farm
...
def fileSend(request, pagename=None, filename=None):
  ...
  if farm.isDomainInFarm(referer_domain, request):
      ...
      allowed = re.search(request.config.referer_regexp, request.http_referer, re.IGNORECASE)
  else:
      allowed = True
  ...
  if do_download:  
      ...
      mimetype = 'application/x-download' # bogus mimetype to force browsers to behave
  request.http_headers([("Content-Type", mimetype), ("Content-Length", length), ("Last-Modified", datestring), ("Content-Disposition", contentstring)])
  #output image
