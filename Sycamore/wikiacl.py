...
class AccessControlList:
    ...
    def may(self, request, username, dowhat):
        ...
        for groupname in self.acl_dict:
            if groupname in ['All', 'Known', 'Banned', 'Admin']:
                continue
        ...
        if ((username in banned_group) or
            (request.remote_addr in banned_group.get_ips())):
            allowed = self.acl_dict_defaults["Banned"][
                ACL_RIGHTS_TABLE[dowhat]]
            return allowed

        if request.user.valid:
          # we fall back to Known behavior if there is something
          # specific to Known on this page
          if self.acl_dict.has_key('Known'):
             allowed = self.acl_dict["Known"][ACL_RIGHTS_TABLE[dowhat]]
             return allowed
        else:
          # we fall back to All behavior if there is something
          # specific to All on this page
          if self.acl_dict.has_key('All'):
             allowed = self.acl_dict["All"][ACL_RIGHTS_TABLE[dowhat]]
             return allowed

        # no specific settings for Known on this page, so let's
        # do what the default for the group is
        defaults_without_special = copy(
            request.config.acl_rights_default.keys())
        ...
        for groupname in ["Known", "All"]:
            if groupname == 'Known' and has_known_setting:
                break
            if groupname == 'All' and has_all_setting:
                break
...
class Group(object):
    def __init__(self, name, request, fresh=False):
        ...
        if self.request.config.acl_rights_default.has_key(self.name):
            self.acl_rights_default = \
                self.request.config.acl_rights_default[self.name]
        else: 
            self.acl_rights_default = \
                self.request.config.acl_rights_default['Known']
    ...
    def _init_groupdict(self):
      if self.groupdict is None:
          ...
          groupdict = None
          if not self.fresh:
            if self.request.req_cache['group_dict'].has_key(
                (self.name, self.request.config.wiki_id)):
              self.groupdict = self.request.req_cache['group_dict'][
                (self.name, self.request.config.wiki_id)]
              return

            if config.memcache:
              groupdict = self.request.mc.get('groupdict:%s' % (
                                              mc_quote(self.name)))

          if groupdict is None:
                groupdict = {}
                d = {'groupname':self.name,
                     'wiki_id': self.request.config.wiki_id}
                self.request.cursor.execute(
                    """SELECT username from userGroups
                       where groupname=%(groupname)s and
                             wiki_id=%(wiki_id)s""", d)
                ...
                if config.memcache and not self.fresh:
                    self.request.mc.add('groupdict:%s' % mc_quote(self.name),
                                        groupdict)

          self.request.req_cache['group_dict'][
            (self.name, self.request.config.wiki_id)] = groupdict
    ...
    def _init_ips(self):
        if self.ips is None:
            ...
            ips = None
            if not self.fresh:
                if self.request.req_cache['group_ips'].has_key(
                    (self.name, self.request.config.wiki_id)):
                    self.ips = self.request.req_cache['group_ips'][
                        (self.name, self.request.config.wiki_id)]
                    return

                if config.memcache:
                    ips = self.request.mc.get(
                        'groupips:%s' % mc_quote(self.name))
            ...
            if ips is None:
                ...
                if results:
                    for item in results:
                      ips[item[0]] = None
                if config.memcache and not self.fresh:
                    self.request.mc.add('groupips:%s' % mc_quote(self.name),
                                        ips)

            self.request.req_cache['group_ips'][
                (self.name, self.request.config.wiki_id)] = ips 
            self.ips = ips
    ...
    def update(self, dict):
        """
        Given a dictionary of the form {username: None ..}, this will update
        the group list.  The group will then contain only the members specified
    ...
    def update_ips(self, dict):
        """
        Given a dictionary of the form {ip_addr: None ..}, this will update the
        ip list.  The group will then contain only the ips specified in the
        dictionary provided as its IP address
        (normal 'users' will still be there).
        """
        self._init_ips()
        add_ips = []
        remove_ips = []
    ...
    def save(self):
        ...
        if self.groupdict is not None:
            for username in self.add_users:
                d['username'] = username
                self.request.cursor.execute(
                    """INSERT into userGroups (username, groupname, wiki_id)
                       values (%(username)s, %(groupname)s, %(wiki_id)s)""",
                    d, isWrite=True)
            ...
            if config.memcache:
                self.request.mc.set('groupdict:%s' % mc_quote(self.name),
                                    self.groupdict)

            self.request.req_cache['group_dict'][
                (self.name, self.request.config.wiki_id)] = self.groupdict

        if self.ips is not None:
            for ip in self.add_ips:
                d['ip'] = ip
                self.request.cursor.execute(
                    """INSERT into userGroupsIPs (ip, groupname, wiki_id)
                       values (%(ip)s, %(groupname)s, %(wiki_id)s)""", d,
                    isWrite=True)
            for ip in self.remove_ips:
                d['ip'] = ip
                self.request.cursor.execute(
                    """DELETE from userGroupsIPs
                       where ip=%(ip)s and groupname=%(groupname)s and
                             wiki_id=%(wiki_id)s""", d, isWrite=True)

            if config.memcache:
                self.request.mc.set('groupips:%s' % mc_quote(self.name),
                                    self.ips)

            self.request.req_cache['group_ips'][
                (self.name, self.request.config.wiki_id)] = self.ips
...
def getACL(pagename, request, fresh=False):
    ...
    if acl_dict is None:
        ...
        request.cursor.execute(
            """SELECT groupname, may_read, may_edit, may_delete, may_admin
               from pageAcls
               where pagename=%(pagename)s and wiki_id=%(wiki_id)s""", d) 
        results = request.cursor.fetchall()
        if results:
            for groupname, may_read, may_edit, may_delete, may_admin in results:
                acl_dict[groupname] = (may_read, may_edit, may_delete,
                                       may_admin) 
    else:
        got_from_mc = True
...
def _sameAsDefaults(groupdict, request):
    ...
    for groupname in request.config.acl_rights_default:
        # we don't let the banned or admin groups be altered on a page
        # because it's pointless
        if groupname != 'Banned' and groupname != 'Admin':
            defaults[groupname] = list(
                request.config.acl_rights_default[groupname])
...
def setACL(pagename, groupdict, request):
    ...
    for groupname in groupdict:
        ...
        request.cursor.execute(
            """SELECT groupname from pageAcls
               where pagename=%(pagename)s and wiki_id=%(wiki_id)s and
                     groupname=%(groupname)s""", d)
        if request.cursor.fetchone():
            request.cursor.execute(
                """UPDATE pageAcls set may_read=%(may_read)s,
                                       may_edit=%(may_edit)s,
                                       may_delete=%(may_delete)s,
                                       may_admin=%(may_admin)s
                   where groupname=%(groupname)s and pagename=%(pagename)s and
                         wiki_id=%(wiki_id)s""", d, isWrite=True)
        else:
            request.cursor.execute(
                """INSERT into pageAcls (groupname, pagename, may_read,
                                         may_edit, may_delete, may_admin,
                                         wiki_id)
                   values (%(groupname)s, %(pagename)s, %(may_read)s,
                           %(may_edit)s, %(may_delete)s, %(may_admin)s,
                           %(wiki_id)s)""", d, isWrite=True)

    # if the settings are the same then we clear them out so master
    # changes can propagate through
    if _sameAsDefaults(groupdict, request):
        request.cursor.execute("""DELETE from pageAcls
            where pagename=%(pagename)s and wiki_id=%(wiki_id)s""", d,
            isWrite=True) 

    if config.memcache:
        # want to clear out when it's the same as the global defaults.
        # this way changes to global settings will affect the page if
        # it's not special in any way, priv-wise.
        if _sameAsDefaults(groupdict, request): 
    ...
    if groupdict:
        request.req_cache['acls'][
            (pagename, request.config.wiki_id)] = AccessControlList(request,
                dict=groupdict)
    else:
        request.req_cache['acls'][
            (pagename, request.config.wiki_id)] = AccessControlList(request)
