...
import __init__
from Sycamore import wikiutil, config, request, caching, wikidb, search, user
...
def build(wiki_name, pname, doprint=False, req=None, super=False):
  ...
  if doprint: print "  -->", pname.encode(config.charset)
      ...
      while True:
        ...
        for num in xrange(0, MAX_THREADS):
          if i >= len(plist):
            break
          pname = plist[i]
          t = threading.Thread(target=clear, args=(wiki_name, pname, doprint, req))
          threads.append(t)
          t.start()
          i += 1
        for t in threads:
      ...
      while True:
        ...
        for num in xrange(0, MAX_THREADS):
            if i >= len(plist):
              break
            pname = plist[i]
            t = threading.Thread(target=build, args=(wiki_name, pname, doprint, req, super))
            threads.append(t)
            t.start()
            i += 1
        for t in threads:
...
if __name__ == "__main__":
  ...
  print "  5) Super-Rebuild all page caches, because you were doing something stupid."
  print "  6) Rebuild the search index (remote -- use this if you care about goodness)."
    ...
    for wiki_name in wiki_list:
      ...
      for pagename in plist:
        ...
        while threading.activeCount() > MAX_REBUILD_THREADS:
           time.sleep(0.05)
        threading.Thread(target=search.add_to_index, args=(page,), kwargs={'do_now':True}).start()
