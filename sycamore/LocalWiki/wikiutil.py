...
def importPlugin(kind, name, function="execute"):
    ...
    or any other directory that exist in LocalWiki or data/plugin
...
def builtinPlugins(kind):
    """
    Gets a list of modules in LocalWiki.'kind'
    
    @param kind: what kind of modules we look for
    @rtype: list
    @return: module names
    """
    plugins =  pysupport.importName("LocalWiki." + kind, "modules")
    if plugins == None:
        return []
    else:
        return plugins
