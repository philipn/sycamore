...
Searches in the Sycamore sources for calls of _() and tries to extract
...
languages. Look into Sycamore.i18n.__init__ for availeable language
names.

The script will run from the moin root directory, where the Sycamore
package lives, or from Sycamore/i18n where this script lives.
...
# Sycamore specific stuff follows
...
class Report:
    ...
    def loadLanguage(self):
        ...
        self.__langDict = pysupport.importName("Sycamore.i18n." + filename, "text")
...
if __name__ == '__main__':
    ...
    # Check that we run from the root directory where Sycamore package lives
    # or from the i18n directory when this script lives
    if os.path.exists('Sycamore/__init__.py'):
        # Running from the root directory
        Sycamore_dir = os.curdir   
    elif os.path.exists(os.path.join(os.pardir, 'i18n')):
        # Runing from i18n
        Sycamore_dir = os.path.join(os.pardir, os.pardir)
    else:
        print __doc__
        sys.exit(1)

    # Insert Sycamore_dir into sys.path
    sys.path.insert(0, Sycamore_dir)
    ...
    for root, dirs, files in os.walk(os.path.join(Sycamore_dir, 'Sycamore')):
        for name in files:
            if name.endswith('.py'):
    ...
    print "This page is generated by `Sycamore/i18n/check_i18n.py`."
