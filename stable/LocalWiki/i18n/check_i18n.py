...
Searches in the LocalWiki sources for calls of _() and tries to extract
...
languages. Look into LocalWiki.i18n.__init__ for availeable language
names.

The script will run from the moin root directory, where the LocalWiki
package lives, or from LocalWiki/i18n where this script lives.
...
# LocalWiki specific stuff follows
...
class Report:
    ...
    def loadLanguage(self):
        ...
        self.__langDict = pysupport.importName("LocalWiki.i18n." + filename, "text")
...
if __name__ == '__main__':
    ...
    # Check that we run from the root directory where LocalWiki package lives
    # or from the i18n directory when this script lives
    if os.path.exists('LocalWiki/__init__.py'):
        # Running from the root directory
        LocalWiki_dir = os.curdir   
    elif os.path.exists(os.path.join(os.pardir, 'i18n')):
        # Runing from i18n
        LocalWiki_dir = os.path.join(os.pardir, os.pardir)
    else:
        print __doc__
        sys.exit(1)

    # Insert LocalWiki_dir into sys.path
    sys.path.insert(0, LocalWiki_dir)
    ...
    for root, dirs, files in os.walk(os.path.join(LocalWiki_dir, 'LocalWiki')):
        for name in files:
            if name.endswith('.py'):
    ...
    print "This page is generated by `LocalWiki/i18n/check_i18n.py`."
